...
const Message = {
	...
	<tr @click="details=details?0:1" :class="prioClasses[msg.PRIORITY]">
		...
		<td :data-l10n-id="'log_priority-'+msg.PRIORITY"></td>
...
diagIndex.addPage('log', {
	...
	<div class="col-xl-2 d-flex flex-column">
		<button v-if="config.desktop" @click="extendNewer" :disabled="follow || !messages.length" class="btn btn-secondary w-100 mb-3" data-l10n-id="log_extendNewer"></button>
			...
			<div class="col col-xl-12">
				<div class="mb-3">
					...
					<select v-model="priority" class="form-select">
						<option v-for="prio in [0,1,2,3,4,5,6,7]" :value="prio" :data-l10n-id="'log_priority-'+prio"></option>
					</select>
				</div>
				<div class="mb-3">
		...
		<button v-if="config.desktop" @click="extendOlder" class="btn btn-secondary w-100 mt-auto mb-3" data-l10n-id="log_extendOlder"></button>
	</div>

	<div class="col-xl h-100 overflow-scroll" id="log">
		<div v-if="!messages.length" class="alert alert-info" data-l10n-id="log_empty"></div>
