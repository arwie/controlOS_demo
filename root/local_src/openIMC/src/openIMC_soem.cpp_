...
template<class type>
int sdoWrite(uint16 slave, uint16 index, uint8 subIndex, const type value) {
	return ec_SDOwrite(slave, index, subIndex, FALSE, sizeof(type), &value, EC_TIMEOUTSAFE);
}

int CDHDsetup(uint16 slave) {
    int retval = 0;

    sdoWrite<int8>(slave, 0x1C12, 0, 0);
    sdoWrite<int16>(slave, 0x1C12, 1, 0x1600);
	sdoWrite<int8>(slave, 0x1C12, 0, 1);

	sdoWrite<int8>(slave, 0x1600, 0, 0);
	sdoWrite<int32>(slave, 0x1600, 1, 0x60400010);
	sdoWrite<int32>(slave, 0x1600, 2, 0x607A0020);
	sdoWrite<int8>(slave, 0x1600, 0, 2);

	sdoWrite<int8>(slave, 0x1C13, 0, 0);
	sdoWrite<int16>(slave, 0x1C13, 1, 0x1A00);
	sdoWrite<int8>(slave, 0x1C13, 0, 1);

	sdoWrite<int8>(slave, 0x1A00, 0, 0);
	sdoWrite<int32>(slave, 0x1A00, 1, 0x60410010);
	sdoWrite<int32>(slave, 0x1A00, 2, 0x60640020);
	sdoWrite<int8>(slave, 0x1A00, 0, 2);

    printf("CDHD slave %d set, retval = %d\n", slave, retval);
    return 1;
}

static void etherCat() {
	thread([]() {for (;;this_thread::sleep_for(chrono::milliseconds(1000))) {
		cout << "status1:"<<bitset<16>(cdhdStatus) << " - pfb:"<<dec<<cdhdPfb  << " - jitter:"<<dec<<syncJitter.count() << " - otgCalcTime:"<<dec<<otgOutput.calculation_duration<<":"<<otgCalcTimeMax  << endl;
		syncJitter = chrono::microseconds::zero();
		otgCalcTimeMax = 0;
	}}).detach();
	...
	if (ec_config_init(FALSE) > 0) {
		...
		printf("ec_slave[1].name = %s", ec_slave[1].name);
		ec_slave[1].PO2SOconfig = CDHDsetup;
		ec_config_map(&IOmap);
		ec_configdc();
	}
